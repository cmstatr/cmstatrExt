...
<script type='text/javascript'>
  ...
  document.getElementById("compute-factors").addEventListener("click", async () => {
    ...
    setTimeout(function () {
      ...
      outArea.value =
        `Factirs for: alpha = ${fA}, n = ${iN}, m = ${iM}\nk1 = ${k1.toFixed(4)}\nk2 = ${k2.toFixed(4)}`;
...
## Details {.tabset}
### Method Details
Based on a user selected qualification sample size ($n$), acceptance
sample size ($m$) and significance level ($\alpha$), the factors
$k_1$ and $k_2$ are calculated. Acceptance limits are set as:
...
The power of this acceptance criteria is investigated through simulation.
In this simulation, 2500 qualification samples are drawn from a standard
normal distribution ($N(\mu, \sigma)$) and acceptance limits are computed
based on each qualification sample.
Next 2500 acceptance samples are drawn from a
$N(\mu-\delta\sigma, \sigma)$ distribution.
Each of the acceptance samples are compared against each of the acceptance
limits and the proportion of acceptance samples rejected are reported.
